...
// ---------------------------------------------------------------------------
// Ligues ranked (basées sur XP global)
// ---------------------------------------------------------------------------
const LEAGUES = [
...
function userLeague(totalXp) {
  let league = LEAGUES[0];
  for (const l of LEAGUES) { if (totalXp >= l.minXp) league = l; }
  const idx = LEAGUES.indexOf(league);
  const next = LEAGUES[idx + 1] || null;
  const progress = next
    ? Math.min(100, Math.round(((totalXp - league.minXp) / (next.minXp - league.minXp)) * 100))
    : 100;
  return { ...league, idx, progress, xpToNext: next ? Math.max(0, next.minXp - totalXp) : 0, next };
}
